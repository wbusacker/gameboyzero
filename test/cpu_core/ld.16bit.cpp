...
TEST(LD_16, POP_BC) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).WillOnce(Return(0xC1));
...
TEST(LD_16, POP_DE) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).WillOnce(Return(0xD1));
...
TEST(LD_16, POP_HL) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).WillOnce(Return(0xE1));
...
TEST(LD_16, POP_AF) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).WillOnce(Return(0xF1));
...
TEST(LD_16, PUSH_HL) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).Times(1).WillOnce(Return(0xE5));
...
TEST(LD_16, PUSH_AF) {
    ...
    EXPECT_CALL(bus, fetch_addr(_)).Times(1).WillOnce(Return(0xF5));
