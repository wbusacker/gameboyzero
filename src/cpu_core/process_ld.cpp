...
void LR35902::process_ld(uint8_t instr){
    uint8_t*    dst_reg;
    uint8_t*    src_reg;
    uint8_t     hl_wr;
    uint16_t    nn_lsb;
    ...
    } else if(check_bits(instr, 0b11010111, 0b00000110) ||
              check_bits(instr, 0b11001111, 0b00001110) ||
    ...
    /* Start using the switch statement */
        ...
        switch(instr){
            ...
            case 0x08: /* Stack Pointer to Immediate Indirect   */
                ...
                nn_msb = memory_bus.fetch_addr(program_counter++);
                nn = (nn_msb << 8) | nn_lsb;
                memory_bus.store_addr(
                    nn,
                    static_cast<uint8_t>(stack_pointer & 0xFF)
                );
                memory_bus.store_addr(
                    nn + 1,
                    static_cast<uint8_t>((stack_pointer >> 8) & 0xFF)
                );
                instr_cycles = 5;
                break;
            ...
            case 0x36: /* Immediate to HL Indirect              */
                memory_bus.store_addr(get_HL_indirect(),
                              memory_bus.fetch_addr(program_counter++));
                instr_cycles = 3;
                break;
